...
let base_url =
  ...
  Cmdliner.Arg.(required & opt (some string) None & info ["base-url"] ~docv:"URL" ~doc)
...
let execute base_url token username channel icon_url icon_emoji attachment_text msg =
  ...
  let module MySlacko = Slacko.MakeSlacko(struct let base_url = base_url end) in
  let open Lwt in
  let token = MySlacko.token_of_string token in
  let channel = MySlacko.channel_of_string channel in
  let chat = MySlacko.Channel channel in
  let msg = MySlacko.message_of_string msg in
  let attachments =
    match attachment_text with
    | None -> None
    | Some text -> Some [MySlacko.attachment ~text ()]
  in
  MySlacko.chat_post_message token chat
    ?username:(username)
